...
#' @param filename filename for file used for copied functions; defaults to
#'   "burgled.R".
#' @details
#'
#' By default, the file `"R/burgled.R"` is created or updated with the
#' definition of the copied objects. The filename can be set as a parameter of
#' `burgle()`.
#'
...
#' @export
burgle <- function(..., license_warning = TRUE, filename = "burgled.R") {
  if (!grepl("\\.R$", filename)) {
    stop("'filename' must end with a '.R' file extension")
  }
  burgled_path <- file.path("R", filename)
  ...
  for (i in seq_along(dots)) {
    ...
    burgle1(fun, pkg, new_name = new_name, burgled_path = burgled_path)
  },
  error = function(e) {
